...
using the expression \code{undefined}, as in:\footnote{%
  \texttt{\{}~\texttt{\}} denotes a ``hole'' in an Agda program.  When a hole
  is present, the program is incomplete.  There are no partial functions in
  Agda, per se---Agda is a ``total'' functional language. Thus, an Agda program
  that can be fully and successfully type-checked cannot have holes. There are
  no non-terminating function calls and recursion is limited to functions that
  provably terminate.}
