...
morphism \code{f\ ::\ a-\textgreater{}b}. First, lets see what
...
\code{b\ -\textgreater{}\ x}). The only way this can be done for all
...
\code{a-\textgreater{}x} is a continuation (the handler). The result
...
it's not true that \code{a\ \textless{}=\ b}; or it exists if
\code{a\ \textless{}=\ b}, and it points from \code{a} to
...
\code{a\ \textless{}=\ b} and \code{b\ \textless{}=\ c} then
\code{a\ \textless{}=\ c}; and the composition is associative. We also
...
\code{a\ \textless{}=\ b} --- in which case it's a one-element set.
Consequently, if \code{a\ \textless{}=\ b}, there exists a single
...
an empty hom-set. In other words, if \code{x\ \textless{}=\ a}
...
\code{x\ \textless{}=\ a} then \code{x\ \textless{}=\ b}.
...
\code{a\ \textless{}=\ b}. Together, we get:
...
\code{a\ \textless{}=\ b} then all elements that are below \code{a}
