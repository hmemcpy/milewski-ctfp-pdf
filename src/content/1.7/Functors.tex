...
itself is a function \code{(a-\textgreater{}b)} --- returning a
function \code{(Maybe\ a\ -\textgreater{}\ Maybe\ b)}; or as a
...
\code{optional\textless{}T\textgreater{}}. Let's define its action on
functions:
...
function \code{a-\textgreater{}b} define a function
\code{List\ a\ -\textgreater{}\ List\ b}:
...
\code{a-\textgreater{}b}, but it can equally well be used in prefix
...
complete type \code{a-\textgreater{}b}. As it stands, it defines a
...
\code{a} and maps it into the type \code{r-\textgreater{}a}. To show
that it's a functor, we want to lift a function
\code{a-\textgreater{}b} to a function that takes
\code{r-\textgreater{}a} and returns \code{r-\textgreater{}b}. These
are the types that are formed using the type constructor
\code{(-\textgreater{})\ r} acting on, respectively, \code{a} and
...
\code{f::a-\textgreater{}b} and a function
\code{g::r-\textgreater{}a}, create a function
\code{r-\textgreater{}b}. There is only one way we can compose the two
...
This combination of the type constructor \code{(-\textgreater{})\ r}
