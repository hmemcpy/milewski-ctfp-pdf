...
In proof assistants like Agda it is possible to write code that expresses
...
record Category : Set where
  field
    -- types
    Obj : Set              -- objects
    Hom : (source : Obj)   -- (homo-) morphisms
        → (target : Obj)   -- arrows between source and target object
        → Set

    -- operations
    id : ∀ {A : Obj} → Hom A A                           -- identity
    _⋆_ : ∀ {A B C : Obj} → Hom B C → Hom A B → Hom A C  -- composition
