...
\emph{D} are \newterm{equivalent} if we can find two functors going back
...
\code{R\ ∘\ L} and the identity functor \code{ID}, and another
between \code{L\ ∘\ R} and the identity functor \code{IC}.
...
shoot an arrow --- the morphism \code{ηd} --- to our target.
...
\code{εc\&apos;} --- from the source to the target.
...
before, we have two functors \code{L\ ::\ D-\textgreater{}C} and
\code{R\ ::\ C-\textgreater{}D}. We pick two arbitrary objects: the
...
component of \code{ηd} to get from \code{d} to
\code{(R\ ∘\ L)\ d}. We get:
...
\code{p::c-\textgreater{}a} and \code{q::c-\textgreater{}b}, there
exists a unique morphism \code{m::c-\textgreater{}(a,\ b)} that
...
in Haskell, we can implement a \code{factorizer} that generates this
...
write the pair formally as \code{\textless{}a,\ b\textgreater{}},
...
represent the pair \code{\textless{}a,\ b\textgreater{}} in a way
...
morphisms \code{\textless{}f,\ g\textgreater{}}:
...
the pair \code{\textless{}a,\ b\textgreater{}} to our product object,
...
\code{\textless{}p,\ q\textgreater{}} in \emph{C×C} that goes from
\code{\textless{}c,\ c\textgreater{}} to
\code{\textless{}a,\ b\textgreater{}}; in other words, a morphism
...
\code{\textless{}a,\ b\textgreater{}}. Notice that any functor from
...
the function object \code{a⇒b} (or \code{ba}). Again, \code{a} is
