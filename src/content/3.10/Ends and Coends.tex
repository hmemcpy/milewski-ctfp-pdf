...
pair of morphisms \code{b-\textgreater{}a} and
\code{a-\textgreater{}c}. Even better, we can use a single morphism to
reach off-diagonal values. For instance, if we have a morphism
\code{f::a-\textgreater{}b}, we can lift the pair
\code{\textless{}f,\ idb\textgreater{}} to go from \code{p\ b\ b} to
\code{p\ a\ b}:
...
Or we can lift the pair \code{\textless{}ida,\ f\textgreater{}} to go
...
are members of the functor category \code{{[}Cop\ ×\ C,\ Set{]}}, is a
family of morphisms:
...
\code{{[}Cop\ ×\ C,\ Set{]}}, the above diagram could be constructed
...
\code{{[}Cop\ ×\ C,\ Set{]}} is indexed by a pair of objects
...
morphism \code{f::a-\textgreater{}b} in \emph{C}, we can connect both
...
\code{h::a-\textgreater{}e} that makes all triangles commute:
...
blog post}). For any function \code{f\ ::\ a\ -\textgreater{}\ b}, the
...
generalization of the constant functor \code{Δc} to a constant
...
diamond when we realize that \code{Δc} lifts all morphisms to one
identity function.
...
Let's just pick one element from the set \code{∫c\ C(F\ c,\ G\ c)}.
...
\code{\textless{}ida,\ G\ f\textgreater{}} using the hom-functor. You
may recall that such lifting is implemented as simultaneous pre- and
post-composition. When acting on \code{τa} the lifted pair gives us:
...
possible functions \code{b-\textgreater{}a}. In Haskell, that would be
...
\code{b-\textgreater{}a} and an element of \code{p\ a\ b}. The
...
their hom-functors are isomorphic. In other words \code{a\ ≅\ b} if
